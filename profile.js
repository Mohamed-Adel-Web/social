...
let editedPostId = "";
let deletePostId = "";
let profileContainer = document.querySelector(".profile-post-info .container");
let profilePostsContainer = document.querySelector(".profile-posts .container");
let urlParam = new URLSearchParams(window.location.search);
let profileId = urlParam.get("postId");
...
function getProfileInfo() {
  ...
  if (window.localStorage.getItem("token") != null) {
    ...
    axios
      .get(`https://tarmeezacademy.com/api/v1/users/${data.id}`)
      .then((response) => {
        let user = response.data.data;
        console.log(user);
        let content = `<div class="row justify-content-center  ">
        <div class="col-9 bg-white shadow-sm text-center ">
    <div class="row p-3">
        <div class="col-lg-2"><img src="${user.profile_image}" class="img-fluid " alt=""></div>
        <div class="col-lg-4">
            <li class="name my-2 ">${user.name}</li>
            <li class="user-name">${user.username}</li>
        </div>
        <div class="col-lg-4">
            <li class="num-of-post text-black-50   "><span class="text-black me-2 fs-3 ">${user.posts_count}</span>posts</li>
        <li class="num-of-comment text-black-50"><span class="text-black me-2 fs-3 ">${user.comments_count}</span>comments</li>
    </div>
    </div>
        </div>
    </div>`;
...
function getProfilePosts() {
  ...
  if (window.localStorage.getItem("token") != null) {
    let profilePostsUrl = `https://tarmeezacademy.com/api/v1/users/${
 ...
 ${element.body}
          ...
          profilePostsContainer.innerHTML += content;
        }
      })
      .catch(function (error) {
        window.alert(error);
...
function getUserPosts(params) {
  profilePostsContainer.innerHTML = "";
  let profilePostsUrl = `https://tarmeezacademy.com/api/v1/users/${profileId}/posts`;
  axios
    .get(`${profilePostsUrl}`)
    .then((response) => {
      let posts = response.data.data;
...
if (window.localStorage.getItem("token") != null) {
  if (
    profileId != null &&
    profileId != JSON.parse(window.localStorage.getItem("user")).id
  ) {
    getUserInfo();
    getUserPosts();
  } else {
